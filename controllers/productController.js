...
const AddProduct = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    const { category_id, name, title, price, price_description, availability, status, buy_link, youtube_link, instructions, description } = req.body;
    ...
    try {
      ...
      if (req.files["additional_images"]) {
        const imageFiles = req.files["additional_images"];
        for (const file of imageFiles) {
          const imageUrl = await uploadToS3(file, folder);
          await db.insert("tbl_product_images", { product_id: productId, image_url: imageUrl });
        }
      }
...
const GetProducts = async (req, res) => {
  ...
  try {
    ...
    for (let product of products) {
      product.images = await db.selectAll("tbl_product_images", "*", `product_id = ${product.id}`);
    }
...
const DeleteProductImage = async (req, res) => {
  const { tenantId, productId, imageId } = req.params;
  if (!checkTenantAuth(req, tenantId)) {
    return res.status(403).json({ error: "UNAUTHORIZED", message: "Unauthorized" });
  }

  try {
    const image = await db.select("tbl_product_images", "*", `id = ${imageId} AND product_id = ${productId}`);
    if (!image) {
      return res.status(404).json({ error: "IMAGE_NOT_FOUND", message: "Image not found" });
    }

    await deleteFromS3(image.image_url);
    await db.delete("tbl_product_images", `id = ${imageId} AND product_id = ${productId}`);
...
const DeleteProduct = async (req, res) => {
  ...
  try {
    ...
    const additionalImages = await db.selectAll("tbl_product_images", "*", `product_id = ${productId}`);
    for (const image of additionalImages) {
      await deleteFromS3(image.image_url);
      await db.delete("tbl_product_images", `id = ${image.id}`);
    }
