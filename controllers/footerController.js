...
const GetSocialLinks = async (req, res) => {
  ...
  try {
    ...
    res.json(socialLinks[0]); // Return the first record (one set of links per tenant)
  } catch (err) {
    console.error(err);
...
const GetDisclaimers = async (req, res) => {
  ...
  try {
    ...
    res.json(disclaimers[0]); // Return the first record (one set of disclaimers per tenant)
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "SERVER_ERROR", message: "Server error" });
  }
